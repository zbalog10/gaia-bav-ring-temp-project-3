...
├── legacy/
├── outputs/
├── src/gaia_bav_ring_temp/
└── tests/
```

## Quick start in PyCharm

1. Open `gaia_bav_ring_temp_project` in PyCharm.
...
4. Copy your CSV files into `data/raw/`:
   - `ring_temp.csv`
   - `sfunc7000_fov1_detrended_newtime.csv`
   - `sfunc7000_fov2_detrended_newtime.csv`

5. Run one experiment:

```bash
gaia-bav-run --config configs/fov1_full.toml
```

Or:

```bash
python -m gaia_bav_ring_temp.cli --config configs/fov2_zoom.toml
```
