...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="Detrending method to use. Default: linear",
    )
    parser.add_argument(
        "--out-subdir",
        type=str,
...
def plot_raw_with_trend(
    ...
    """Plot raw averaged temperatures with fitted trend lines only."""
    n = len(feature_cols)
    fig, axes = plt.subplots(
        n,
        1,
    ...
    axes[-1].set_xlabel(obmt_col)
    fig.suptitle("Raw averaged temperatures with fitted trend", fontsize=14)
...
def main() -> None:
    ...
    raw_trend_plot_path = out_dir / "temperature_raw_with_trend.png"
    plot_raw_with_trend(
        enriched,
        feature_cols=feature_cols,
        obmt_col=config.los_obmt_col,
        out_path=raw_trend_plot_path,
        show=False,
    )
    ...
    summary_json = {
        ...
        "raw_with_trend_plot_file": str(raw_trend_plot_path),
